...
Web Interface
    ...
    Create harvests via API or CLI.
    Monitor jobs and open details from the dashboard.
...
Example usage:
    ...
    python -m cli.main --url https://example.com --mode full --depth 2 --output ./output

API Endpoints

Using Django REST Framework:

    POST /api/harvest/: Submit job. Body: { "url": "https://example.com", "options": {"mode": "full", "depth": 2} }. Returns job data with ID.
    GET /api/jobs/: List jobs (paginated).
    GET /api/jobs/<id>/: Check status.
    GET /api/jobs/<id>/result/: Fetch result payload.
    GET /api/results/: List results (paginated).
    GET /api/tech-detect/?url=https://example.com: Quick tech scan without full harvest.
    POST /api/tech-detect/: Body: { "url": "https://example.com" }.
